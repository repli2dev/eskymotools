...
abstract class GeneralEntity extends EskymoListenableObject implements IEntity {
	...
	public final function addOnLoadPersistedListener(IListener $listener) {
		$this->addListener(IEntity::EVENT_LOAD_PERSISTED, $listener);
	}
	...
	public final function loadDataFromArray(array $source, $annotation = NULL) {
		...
		if ($this->loadId($source)) {
			$this->callListeners(IEntity::EVENT_LOAD_PERSISTED, new EntityEvent($this));
		}
		return $this;
	}
	 ...
	 *
	 * @param array $source
	 * @return TRUE if the ID has been loaded, otherwise FALSE.
	 */
